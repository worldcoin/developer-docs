...
For returning-user checks, use [session proofs](/world-id/idkit/session-proofs)
with `CredentialRequest("selfie")`. Create a session once, then prove the saved
session on subsequent checks. Sessions use constraints rather than presets.

For a one-time check, use the `selfieCheck()` preset shown below. It requests
World ID 4.0 and always disables legacy fallback, even if `allow_legacy_proofs`
is set to `true`.
...
### One-time Selfie Check

Use this flow when each user should complete a particular action once. For
repeated verification of the same user, follow the [session guide](/world-id/idkit/session-proofs).

<CodeGroup title="One-time Selfie Check">
```typescript title="JavaScript"
...
Use separate actions for genuinely separate one-time operations, such as distinct
reward claims. For recurring checks, reuse a session instead of creating a new
action for every verification.
