...
We also use the `--rpc-url` flag to specify the RPC URL of the World Chain Sepolia network and the `--private-key` flag to specify the private key of the wallet we generated earlier. Current versions of Foundry default `forge create` to a dry run, so the `--broadcast` flag is required to actually send the deployment transaction.
