...
4. The App Clip persists the selection to a shared container, then prompts the user to install World App.
5. The user installs World App from the App Store.
6. World App reads the shared container, completes account creation onboarding, and walks the user through credential enrollment (including NFC scanning).
7. The proof consent appears, the user approves, and the proof is returned to your app.

Because the App Clip and World App share a data container, the user's country and document selection survive the install.
...
sequenceDiagram
    ...
    App->>User: IDKit request (document credential)
    User->>AC: Presented with App Clip
    AC->>User: Country & document selection
    AC->>AC: Persist selection to shared container
    AC->>User: Prompt to install World App
    User->>Store: Download World App
    WA->>WA: Read shared container
    WA->>User: Account onboarding
    WA->>User: Credential enrollment (incl. NFC scan)
    WA->>User: Proof consent
    User->>WA: Approve
    WA-->>App: Proof returned
```
