...
> **Want an agent to do this for you?** Paste [`docs.world.org/world-id/skill.md`](/world-id/skill.md) into Claude (or any MCP-capable coding agent) with *"add World ID to my app."* The skill walks the agent through every step on this page, sets up the [Developer Portal MCP](https://github.com/worldcoin/developer-portal/tree/main/web/api/mcp), and captures the one-time signing key safely. See the [overview](/world-id/overview) for the full agent quickstart.
