...
description: Real-time, on-chain creditworthiness for World accounts. Look up by wallet address or World username to get borrower state and a credit score.
---

World proves there's a real human behind an account. The Credit API tells you whether you can trust them. It provides real-time creditworthiness data for [over half a million](https://credit.cash/analytics) World accounts that have borrowed on [Credit](https://www.notion.so/ebdd8475db3238254fca5b25ccba266a?pvs=21). This lets you focus your app and incentives on trustworthy users who are less likely to game or farm your system.

Unlike traditional credit bureaus with opaque scoring models, this API calculates credit scores purely from verifiable on-chain data. Every data point comes directly from smart contract events.
