...
<Note>
  Only `groupId` `1` (Orb credentials) is supported for RPs.
</Note>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td className="p-2 align-middle">
        Determines which Credential Type to verify against. Only{" "}
        <code>1</code> (Orb credentials) is supported for RPs.
      </td>
    </tr>
    <tr>
      <td className="p-2 align-middle">
    ...
    <tr>
      ...
      <td className="p-2 align-middle">
        ...
        obtained from IDKit as a hex string (<code>nullifier</code> in IDKit
        4.x v3 responses, <code>nullifier_hash</code> in older versions), and
        must be converted to a <code>uint256</code> before passing it to the{" "}
