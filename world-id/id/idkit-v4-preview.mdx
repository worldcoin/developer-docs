...
<Note>
  `IDKit.init()` initializes the WASM module for browser use. It's safe to call
  multiple times - initialization only happens once. Server-side functions like
  `signRequest` use pure JS and need no initialization.
</Note>
...
<Note>
  If you're using the React package (`@worldcoin/idkit`), the same subpath
  export is available at `@worldcoin/idkit/signing`.
</Note>
...
A lower-level `hashToField` is also available if you need the raw bytes:

```typescript
import { hashToField } from "@worldcoin/idkit-core/hashing";

const fieldElement: Uint8Array = hashToField(new Uint8Array([1, 2, 3])); // 32 bytes
```

<Note>
  Both `hashSignal` and `hashToField` are also available from the main entry
  point (`@worldcoin/idkit-core`) but that import pulls in the full WASM module.
  Prefer the `/hashing` subpath when you only need hashing.
</Note>

## Subpath exports

Both `@worldcoin/idkit-core` and `@worldcoin/idkit` (React) expose subpath exports for signing and hashing. These are pure JS — no WASM initialization required.
...
// React package (re-exports from core)
import { signRequest } from "@worldcoin/idkit/signing";
import { hashSignal } from "@worldcoin/idkit/hashing";
```

This is especially useful in server-side contexts (e.g. Next.js API routes) where importing from the main entry point would pull in React hooks or WASM dependencies.
