...
This is the same `IDKitHookResult<T>` shape returned by `useIDKitInviteCodeRequest` and `useIDKitSession`, and shared by the equivalent widget-based flows.
...
React hooks/widgets take `preset` directly in config for common single-credential requests. `constraints` is an equally valid, mutually-exclusive alternative for custom/multi-credential requests — a config takes either `preset` or `constraints`, never both. Session hooks/widgets (`useIDKitSession`, `IDKitSessionWidget`) require `constraints`; they have no `preset` option.
