...
Choose the proof flow based on whether the user should verify once or return for
future checks:

| Your use case | Flow |
| --- | --- |
| Each user may complete an action once, such as claiming a reward. | A uniqueness request. Follow the steps below and store the nullifier. |
| The same user needs to verify again, including repeated Selfie Check. | [Session proofs](/world-id/idkit/session-proofs). Reuse a verified `session_id` across checks. |

Both flows share installation, Portal setup, RP signing, and backend verification.
For sessions, use the [session guide](/world-id/idkit/session-proofs) for signing
without an action, creating and proving sessions, and storing session results.
...
[session verification and storage guidance](/world-id/idkit/session-proofs#verify-and-authorize-on-your-backend).
