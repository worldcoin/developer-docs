...
export async function processReferral(data: {
  ...
  // a unique referred_user_id column) before crediting anything. If two
  // requests race, only one INSERT succeeds.
  const claimed = await claimReferral(data.newUserId, referrer.id);
  if (!claimed) {
    return { success: false, reason: "already_referred" };
  }

  // Credit both users. If a credit fails after the claim succeeds, retry the
  // credit step alone — the unique claim already prevents a duplicate credit.
  await Promise.all([
...
<Note>
If you use the canonical [Core Event Set](/mini-apps/growth/analytics#2-%C2%B7-core-event-set-6-lines-of-code), `invite_link_created`/`invite_link_clicked` map to `invite_sent`/`invite_accepted` there. Pick one taxonomy and use it consistently.
</Note>
