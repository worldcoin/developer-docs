...
If you build an agent that calls x402 APIs, use [`createAgentkitFetch`](/agents/agent-kit/sdk-reference#createagentkitfetchoptions) so the agent tries AgentKit verification before paying. If you cannot change the agent's HTTP client, add the `agentkit-x402` skill:
...
<CardGroup cols={2}>
  ...
  <Card
    title="SDK Reference"
    icon="code"
    href="/agents/agent-kit/sdk-reference#createagentkitfetchoptions"
  >
    Wrap x402 calls with AgentKit before payment fallback.
  </Card>
</CardGroup>
