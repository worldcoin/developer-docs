...
<Note>
**TypeScript setup**: `@worldcoin/minikit-js`'s shipped types reference `window.WorldApp` without declaring it, which fails under `"skipLibCheck": false`. Set `"skipLibCheck": true` in your `tsconfig.json` (or add your own `Window.WorldApp` shim) so the package's types compile.
</Note>
