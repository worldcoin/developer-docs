...
<Warning>
`poll()` runs an unbounded loop with no built-in timeout, and it **throws** (`Error("Transaction failed")`) if the user operation's status resolves to `failed`. Always call it inside a `try`/`catch`, and wrap it in your own timeout/abort logic if you need an upper bound on wait time.
</Warning>
...
const onClick = async () => {
  ...
  try {
    // Optionally race against your own timeout — poll() has no built-in one.
    const { receipt } = await poll(result.data.userOpHash);
    // receipt contains the final transaction receipt
  } catch (error) {
    // poll() throws if the user operation status resolves to "failed"
...
if (status.status === "success") {
  ...
  console.error("User operation failed:", status.error);
}
```
</CodeGroup>
