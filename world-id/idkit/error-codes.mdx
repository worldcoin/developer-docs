...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td className="p-2 align-middle">Requested feature is not enabled for the app. Available on Kotlin and JS/React; not yet on Swift (see availability note below).</td>
      <td className="p-2 align-middle">Offer fallback credential policy or explain requirement, same as <code>credential_unavailable</code>.</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td className="p-2 align-middle">Inspect the callback and backend verification response. Do not use this code alone to decide whether the issue is in the integration or credential availability.</td>
    </tr>
    <tr>
...
In JS and React, match these with `IDKitErrorCodes`. Kotlin and Swift expose the same raw values through their `IDKitErrorCode` enums, with two exceptions: `invalid_rp_id_format` is JS/React-only, and `feature_unavailable` is currently Kotlin/JS-only until the next Swift release.
