...
to be fungible across the Superchain using the official `SuperchainTokenBridge`. This standard builds
upon the existing [ERC20 token standard](https://docs.openzeppelin.com/contracts/4.x/erc20) and implements the `IERC7802` interface, which includes
two key properties: allowing the `SuperchainTokenBridge` to call `crosschainMint` and `crosschainBurn` functions (other callers may also be allowed),
and ensuring the token is deployed at the same address on every chain in the Superchain.

The `SuperchainTokenBridge` is a predeploy that works as an abstraction on top of the `L2ToL2CrossDomainMessenger` for token bridging.
