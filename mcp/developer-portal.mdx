...
```text
Create a Mini App named Example, upload the logo and content card images I provide, fill the app store metadata, and submit it for review after confirming the final values with me.
```
...
Do not write image URLs directly with `configure_mini_app`; image metadata stores filenames, not public URLs.
