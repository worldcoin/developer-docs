...
`.constraints(...)` builds custom World ID 4.0 credential requests, including composite trees (e.g. Orb OR (Passport AND MNC)), using `CredentialRequest`, `any`, `all`, and `enumerate`. It's the only finalizer `IDKit.createSession()`/`IDKit.proveSession()` builders accept — `.preset()` throws on a session builder.
