...
description: "Deploy the World ID on-chain template to a World Chain Sepolia fork with Foundry; configure env, contracts, and local web."
...
- [pnpm](https://pnpm.io/) (or equivalent like npm, yarn or bun)
...
Once the fork is running, you can deploy the World ID template app to your local World Chain Sepolia fork using the Foundry CLI:

```bash
# cd into the contracts directory
cd contracts
forge create --rpc-url http://localhost:8545 --private-key 0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80 --broadcast src/Contract.sol:Contract --constructor-args $WORLD_ID_ROUTER $NEXT_PUBLIC_APP_ID $NEXT_PUBLIC_ACTION
```

This command will deploy the `Contract` contract to your local World Chain Sepolia fork using the provided RPC URL and private key. The `--broadcast` flag is required because current versions of Foundry default `forge create` to a dry run.
