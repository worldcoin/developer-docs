...
// Published to GitHub Packages, not Maven Central — you need the repository
// + credentials block too. Full setup: /world-id/idkit/kotlin#install
dependencies {
...
const request = await IDKit.request({
  ...
  environment: "production", // "staging" tests with the simulator; "sandbox" tests against a real World ID app — see /world-id/sandbox/what-is-sandbox
...
> Advanced: `.preset(...)` covers the common cases shown above. For custom multi-credential requests (e.g. Orb OR (Passport AND MNC)), finalize the builder with `.constraints(...)` instead, using the `any`/`all`/`enumerate`/`CredentialRequest` helpers exported by `@worldcoin/idkit-core`.
