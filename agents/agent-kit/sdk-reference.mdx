...
`lookupHuman` returns `null` both when the wallet is unregistered and when the lookup itself fails (RPC timeout, network error, public RPC outage) — failures are swallowed internally and indistinguishable from "not registered" by return value alone. To alert on lookup failures separately, wrap `rpcUrl`/`client` with your own timeout and circuit breaker.
