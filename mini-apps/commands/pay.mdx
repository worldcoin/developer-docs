...
Import the `Tokens` enum rather than hardcoding symbol strings — note that some enum values differ from their display name (e.g. USDC's value is `"USDCE"`):
...
type MiniKitPayOptions = {
  ...
  fallback?: () => Promise<PayResult> | PayResult;
};
```
</CodeGroup>
