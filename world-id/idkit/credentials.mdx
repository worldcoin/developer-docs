...
Use `selfieCheck()` to request Selfie Check with World ID 4.0. This preset always
disables legacy fallback, even if `allow_legacy_proofs` is set to `true`.
...
For repeated uniqueness checks, use a different action for each check and enforce
nullifier uniqueness on your backend. For recurring verification of the same
user, see [session proofs](/world-id/4-0-migration#recurring-verifications-and-new-credential-checks).
