...
[Codex](https://www.codex.io/) is a blockchain data API that provides real-time and historical DeFi data across 80+ networks via GraphQL. It offers access to token prices, DEX trades, liquidity pools, wallet activity, and analytics through a single GraphQL endpoint, with real-time updates via WebSocket subscriptions and webhooks.

Codex features:
- GraphQL API with 73 query operations
- Real-time WebSocket subscriptions for price and trade events
- Webhook-based push notifications for on-chain events
