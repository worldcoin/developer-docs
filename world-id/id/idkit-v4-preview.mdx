...
The `result` inside a successful `pollUntilCompletion()` response is shaped exactly as the Developer Portal's [`/v4/verify`](/api-reference/developer-portal/verify) endpoint expects - just forward it directly:
...
On success, `completion.result` can be forwarded directly to the Developer Portal [`/v4/verify`](/api-reference/developer-portal/verify) endpoint - no transformation needed.
