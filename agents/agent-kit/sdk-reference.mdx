...
Use this helper in agents that call paid x402 APIs. It wraps a base `fetch`, inspects `402 Payment Required` responses, and retries once with a signed `agentkit` header when the response advertises `extensions.agentkit`.
...
The helper does not create x402 payments. It returns the original response unchanged when AgentKit is unavailable or cannot be used, so your existing x402 payment client can handle fallback.
...
Creates the base64 `agentkit` HTTP header from an AgentKit extension and signer. Use this lower-level helper when your HTTP client cannot use `createAgentkitFetch`.
