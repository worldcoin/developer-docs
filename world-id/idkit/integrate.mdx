...
<Note>
  Migrating from IDKit `2.x` or another pre-v4 release? See
  [Migrate from IDKit &lt;4 to IDKit v4](/world-id/idkit/migrate-to-v4).
</Note>
...
<Note>
  For RP signature generation, use `@worldcoin/idkit-server` (JavaScript/TypeScript)
  or `github.com/worldcoin/idkit/go/idkit` (Go). See [Go SDK reference](/world-id/idkit/go)
  for Go backend usage.
</Note>
<Warning>
