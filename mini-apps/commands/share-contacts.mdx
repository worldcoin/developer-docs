...
Use `MiniKit.shareContacts()` to open the World App contact picker. The named `getContacts` export from `@worldcoin/minikit-js/commands` is an alias of that module's `shareContacts` function, with the same options, result, and error codes.
