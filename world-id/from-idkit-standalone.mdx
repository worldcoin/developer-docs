...
<Note>
  Once you drop World ID 3.0 compatibility, migrate to the v4-native presets: `proofOfHuman` (replaces `orbLegacy`) and `passport` (NFC/document credentials) — both return World ID 4.0 credentials with automatic legacy fallback. See [Configure Credentials](/world-id/idkit/credentials).
</Note>
