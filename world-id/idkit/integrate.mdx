...
<Note>
  Building a cross-device flow (e.g. desktop browser ↔ phone)? See
  [Invite-code mode](/world-id/idkit/invite-code) for the typeable 6-character
  code alternative to QR scanning.
</Note>
