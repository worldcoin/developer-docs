...
<Note>
**TypeScript setup**: the shipped types reference `window.WorldApp` without declaring it, which fails under `"skipLibCheck": false`. Set `"skipLibCheck": true` in `tsconfig.json` (or add your own `Window.WorldApp` shim).
</Note>
