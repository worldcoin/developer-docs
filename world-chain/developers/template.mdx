...
cd contracts
forge create --rpc-url http://localhost:8545 --private-key 0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80 src/Contract.sol:Contract --constructor-args $WORLD_ID_ROUTER $NEXT_PUBLIC_APP_ID $NEXT_PUBLIC_ACTION --broadcast
```

This command will deploy the `Contract` contract to the World Chain Sepolia network using the provided RPC URL and private key. The `--broadcast` flag is required because current versions of Foundry default `forge create` to a dry run.
