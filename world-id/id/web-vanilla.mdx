...
description: "Use IDKit Standalone to add World ID verification to vanilla JavaScript apps with Widget or Session API."
...
app.post("/api/verify", async (req, res) => {
  ...
  try {
    const verifyRes = await fetch(
      "https://developer.worldcoin.org/api/v2/verify/app_your_app_id",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
