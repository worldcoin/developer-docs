...
- Returns `false` if the address is not verified, or if the verification check itself fails (errors are caught internally, logged to the console, and swallowed rather than thrown)
