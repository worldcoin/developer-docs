...
<Note>
  Only the `selfieCheckLegacy` preset is supported today. The docs will be updated as support is added for other credentials.
</Note>

## When to use it

Use invite-code mode to expedite onboarding users who don't yet hold the credential you're requesting — World App walks them through getting it. It also fits cross-device flows, where typing a short code is more robust than scanning a QR.
