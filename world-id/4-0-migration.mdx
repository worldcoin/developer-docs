...
**Migration approach:** Use [session proofs](/world-id/idkit/session-proofs). Create a session once, save the verified `session_id` against the application account, and prove that same session for later checks. The session guide covers creation, returning-user verification, backend checks, and replay protection.

Do not carry over a uniqueness nullifier as a persistent user identifier. Session responses use `session_nullifier` for per-proof replay protection and `session_id` for continuity.
