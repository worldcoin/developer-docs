...
description: "Add Agent Kit to an x402 server so human-backed agents can access your protected routes."
"og:image": "https://raw.githubusercontent.com/worldcoin/developer-docs/main/images/docs/docs-meta.png"
"og:description": "Use Agent Kit to recognize agents backed by real, World ID-verified humans."
"twitter:image": "https://raw.githubusercontent.com/worldcoin/developer-docs/main/images/docs/docs-meta.png"
---

Agent Kit lets an x402 server recognize when a request comes from an agent backed by a World ID-verified human. Use this page for the shortest path to a working integration. For `discount` mode, custom AgentBook deployments, Solana support, or low-level APIs, jump to the [SDK Reference](/world-id/agents/agent-kit/sdk-reference).

This guide uses one default path throughout:

- World Chain payments on `eip155:480`
- Agent registration on Worldchain
- AgentBook lookup pinned to Worldchain with `createAgentBookVerifier({ network: "world" })`
...
The example below shows the maintained Hono wrapper path. Agent Kit itself is not Hono-only: Express and Next.js route handlers can use the same hooks and low-level helpers from the [SDK Reference](/world-id/agents/agent-kit/sdk-reference).
...
Need `discount` mode, Solana, custom AgentBook deployments, or the low-level validation helpers? Continue to the [SDK Reference](/world-id/agents/agent-kit/sdk-reference).
