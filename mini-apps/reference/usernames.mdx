...
<Note>
Neither method throws for an unknown address or username—the underlying request can 404, in which case the promise still resolves, with `walletAddress`, `username`, and `profilePictureUrl` all `undefined`. Check for a falsy `walletAddress` before using the result.
</Note>
