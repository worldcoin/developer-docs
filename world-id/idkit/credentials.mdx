...
<Warning>
  To use Selfie Check (Beta), [request access](mailto:developers@toolsforhumanity.com)
  so the feature flag can be enabled for your app.
</Warning>

Once enabled, use the `selfieCheckLegacy` preset to request Selfie Check. Anyone
with World App can complete the flow: no Orb, passport, document, or other
