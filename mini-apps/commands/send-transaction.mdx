...
<Warning>
`poll()` runs an unbounded loop with no built-in timeout, and it **throws** (`Error("Transaction failed")`) if the user operation's status resolves to `failed`. Call it inside a `try`/`catch`, and add your own timeout/abort logic for an upper bound on wait time.
</Warning>
...
const onClick = async () => {
  ...
  try {
    // poll() has no built-in timeout — race your own if needed.
