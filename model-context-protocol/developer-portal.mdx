...
If you see this with a key you believe is correct, check the secure storage where you saved it at creation (it's shown only once and the portal can't display it again) — if it's genuinely lost, generate a new key and update every client using the old one. The endpoint only accepts `POST`; `GET` returns HTTP 405 with `allow: POST, OPTIONS`.
