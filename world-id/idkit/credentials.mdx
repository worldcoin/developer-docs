...
Preset helpers cover common credential requests: `proofOfHuman`, `passport`, `mnc`, `selfieCheckLegacy`, `identityCheck`, and the legacy presets.
...
Use `mnc` for the current World ID 4.0 My Number Card (MNC) credential, for users verifying with Japan's My Number Card. The preset also includes legacy document fallback for users without a World ID 4.0 proof.
