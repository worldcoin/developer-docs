...
<Note>
`@worldcoin/minikit-js` does not ship a `Window.WorldApp` type augmentation, so TypeScript will not know about this property out of the box. Declare it yourself, matching the shape below:
