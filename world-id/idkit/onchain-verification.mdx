...
<Warning>
  Never pass a raw, unreduced `keccak256(action)` — it exceeds the BN254 scalar field and reverts with `PublicInputNotInField`. Reduce it with `hash_to_field` (`keccak256(action) >> 8`) first.
</Warning>
