...
For a one-time Selfie Check, use `selfieCheck(signal)`. It requests World ID 4.0 and always
disables legacy fallback. See [Configure Credentials](/world-id/idkit/credentials#selfie-check)
for response verification and repeated-check guidance.

```kotlin
import com.worldcoin.idkit.selfieCheck

val request = IDKit.request(config).preset(selfieCheck(signal = "user-123"))
```
