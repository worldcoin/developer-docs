...
Use this page when you need the full Agent Kit surface area. For the shortest path to a working setup, start with [Integrate Agent Kit](/world-id/agents/agent-kit/integrate).

## Exported surface

`@worldcoin/agentkit` exports:

- Constants: `AGENTKIT`, `AgentkitPayloadSchema`, `SOLANA_MAINNET`, `SOLANA_DEVNET`, `SOLANA_TESTNET`
- Core server APIs: `declareAgentkitExtension`, `agentkitResourceServerExtension`, `createAgentkitHooks`, `createAgentBookVerifier`
- Validation and verification helpers: `parseAgentkitHeader`, `validateAgentkitMessage`, `verifyAgentkitSignature`, `buildAgentkitSchema`
- Chain helpers:
  - EVM: `formatSIWEMessage`, `verifyEVMSignature`, `extractEVMChainId`
  - Solana: `formatSIWSMessage`, `verifySolanaSignature`, `decodeBase58`, `encodeBase58`, `extractSolanaChainReference`
- Storage: `InMemoryAgentKitStorage`
- Types: `AgentkitExtension`, `AgentkitExtensionInfo`, `AgentkitExtensionSchema`, `AgentkitMode`, `AgentkitPayload`, `CompleteAgentkitInfo`, `DeclareAgentkitOptions`, `SignatureScheme`, `SignatureType`, `AgentkitValidationResult`, `AgentkitValidationOptions`, `AgentkitVerifyResult`, `SupportedChain`, `AgentBookVerifier`, `AgentBookOptions`, `AgentBookNetwork`, `AgentKitStorage`, `CreateAgentkitHooksOptions`, `AgentkitHookEvent`
