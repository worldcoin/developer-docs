...
Run the following command and follow the instructions to create a new mini app. This installs dependencies with `npm`; if you prefer `pnpm`, see [Manual Installation](#manual-installation) below.
...
3. Or, in a React component, read the same state reactively with the `useMiniKit()` hook (this is what the official starter template itself uses):

```tsx
import { useMiniKit } from "@worldcoin/minikit-js/minikit-provider";

export function AuthButton() {
  const { isInstalled, user } = useMiniKit();

  if (!isInstalled) return null;

  return <span>{user?.username ?? "Not signed in"}</span>;
}
```
...
<Tabs>
  <Tab title="Claude Code">
    ...
    ```
    `--scope project` writes a shareable `.mcp.json` to your project (recommended); omit it and the server is instead registered only for you, in your global Claude config.
  </Tab>
  <Tab title="Codex">
    ```bash
