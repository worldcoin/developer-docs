...
```json
{
  ...
  "associated_data_commitment": "<field-element-hex>",
...
| `issuer_version` | `u8` | Issuer-specific versioning for the credential format. |
...
| `claims` | `FieldElement[]` | Up to 15 claim commitments (a 16th all-zero capacity slot is reserved internally and stripped on deserialization). Unused indices are the zero field element. |
| `associated_data_commitment` | `FieldElement` | Poseidon2 commitment to issuer-defined associated data. The associated data itself is not included. Previously named `associated_data_hash`; that name is a deprecated alias slated for removal. |
...
- **FieldElement** values (`sub`, `claims`, `associated_data_commitment`) are hex strings with a `0x` prefix and 64 hex characters.
...
- The PoH credential has no associated data, so `associated_data_commitment` is the zero field element.
